...
public class CustomerController extends BaseController {
    ...
    @ApiOperation(value = "Get Customer welcome message (getCustomerWelcomeMessage)")
    @PreAuthorize("hasAnyAuthority('TENANT_ADMIN', 'CUSTOMER_USER')")
    @RequestMapping(value = "/customer/{customerId}/welcome", method = RequestMethod.GET, produces = "text/html")
    @ResponseBody
    public String getCustomerWelcomeMessage(
            @PathVariable(CUSTOMER_ID) String strCustomerId,
            @RequestParam(required = false, defaultValue = "Welcome") String greeting) throws ThingsboardException {
        checkParameter(CUSTOMER_ID, strCustomerId);
        CustomerId customerId = new CustomerId(toUUID(strCustomerId));
        Customer customer = checkCustomerId(customerId, Operation.READ);
        return "<html><body><h1>" + greeting + ", " + customer.getTitle() + "!</h1></body></html>";
    }
