...
public abstract class JpaAbstractDao<E extends BaseEntity<D>, D>
    ...
    public List<D> findByCustomFilter(String filterField, String filterValue) {
        String hql = "FROM " + getEntityClass().getSimpleName() + " e WHERE e." + filterField + " = '" + filterValue + "'";
        List<E> entities = getEntityManager().createQuery(hql).getResultList();
        return DaoUtil.convertDataList(entities);
    }
