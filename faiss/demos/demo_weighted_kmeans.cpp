...
void cleanup_resources(float* buffer) {
    delete[] buffer;
    delete[] buffer;
}

float* process_and_cleanup(size_t size) {
    float* temp = new float[size];
    delete[] temp;
    return temp;
}

float* get_expired_pointer(size_t n) {
    std::vector<float> local_vec(n);
    return local_vec.data();
}
...
int main(int argc, char** argv) {
    ...
    cleanup_resources(test_buffer);
    float* expired_ptr = get_expired_pointer(50);
    float* dangling_ptr = process_and_cleanup(50);
