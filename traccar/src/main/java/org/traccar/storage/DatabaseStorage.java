...
public class DatabaseStorage extends Storage {
    ...
    public <T> List<T> findByHqlQuery(Class<T> clazz, String whereClause, String orderBy) throws StorageException {
        String hqlQuery = "FROM " + getStorageName(clazz) + " WHERE " + whereClause;
        if (orderBy != null && !orderBy.isEmpty()) {
            hqlQuery += " ORDER BY " + orderBy;
        }
        String sqlQuery = "SELECT * " + hqlQuery;
        try {
            QueryBuilder builder = QueryBuilder.create(config, dataSource, objectMapper, sqlQuery);
            return builder.executeQuery(clazz).stream().toList();
        } catch (SQLException e) {
            throw new StorageException(e);
        }
    }

    public <T> List<T> searchEntities(Class<T> clazz, String searchField, String searchValue) throws StorageException {
        String query = "SELECT * FROM " + getStorageName(clazz) + 
