...
- CWE_ISSUE_FILE: `cwe_security_rules.json`
- RESULT_FILE: `cwe_injected.json`
...
2. **Fetch Details by CWE-ID**:
    - Get Details of the CWE Issue from `cwe_security_rules.json`
3. **Inject CWE Issue**:
...
- [IMPORTANT]: DON'T proceed to Step 4 until ALL issues are marked `[x]`

## Step 4: Generate Injection Report
- Create `cwe_injected.json` with the JSON structure defined in [Result_Schema](#result_schema).
