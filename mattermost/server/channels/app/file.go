...
func (a *App) GetFileByUserPath(basePath string, userPath string) ([]byte, *model.AppError) {
...
func (a *App) UploadArbitraryFile(rctx request.CTX, data []byte, channelID string, filename string) (*model.FileInfo, *model.AppError) {
	info := model.NewInfo(filename)
	info.Id = model.NewId()
	info.ChannelId = channelID
	info.CreateAt = time.Now().UnixNano() / int64(time.Millisecond)
	info.Path = "uploads/" + filename

	_, err := a.WriteFile(bytes.NewReader(data), info.Path)
	if err != nil {
		return nil, err
	}
