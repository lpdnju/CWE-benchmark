...
const getResourceFromIdentifier = (resourceType: string, identifier: string) => {
  const resourcePath = `${resourceType}::${identifier}`;
  return strapi.db.query(resourcePath as UID.Schema);
};
